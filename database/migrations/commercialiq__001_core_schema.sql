-- CommercialIQ App #4 — Project Hub core schema
-- Scope: commercialiq.* only. Do not run before hub.assert_app_scope('commercialiq','commercialiq') succeeds.

create schema if not exists commercialiq;

revoke all on schema commercialiq from public;
revoke all on schema commercialiq from anon;
revoke all on schema commercialiq from authenticated;
grant usage on schema commercialiq to postgres;
...
create index if not exists commercialiq_transactions_date_idx on commercialiq.transactions(transaction_date);
create index if not exists commercialiq_transactions_customer_idx on commercialiq.transactions(customer_id);
create index if not exists commercialiq_transactions_product_idx on commercialiq.transactions(product_id);
create index if not exists commercialiq_transactions_region_idx on commercialiq.transactions(region);
...
revoke all on all tables in schema commercialiq from public;
revoke all on all tables in schema commercialiq from anon;
revoke all on all tables in schema commercialiq from authenticated;

alter default privileges in schema commercialiq revoke all on tables from public;
alter default privileges in schema commercialiq revoke all on tables from anon;
alter default privileges in schema commercialiq revoke all on tables from authenticated;

comment on schema commercialiq is 'CommercialIQ App #4 isolated schema in shared Project Hub.';
comment on table commercialiq.customers is 'CommercialIQ synthetic/demo customer dimension.';
comment on table commercialiq.transactions is 'CommercialIQ synthetic/demo commercial transactions.';
